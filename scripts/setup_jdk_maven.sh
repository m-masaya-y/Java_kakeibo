...
# 2. JDK インストール
# -----------------------------
echo "==> JDK $JDK_VERSION をダウンロードしてインストール"
JDK_TEMP="$HOME/.jdk-temp"
mkdir -p $JDK_TEMP
cd $JDK_TEMP

# Intel / Apple Silicon 共通 URL
...
sudo tar -xzf jdk.tar.gz -C $JDK_INSTALL_DIR
...
mkdir -p $MAVEN_TEMP
cd $MAVEN_TEMP
curl -L -o apache-maven.tar.gz https://archive.apache.org/dist/maven/maven-3/$MAVEN_VERSION/binaries/apache-maven-$MAVEN_VERSION-bin.tar.gz
tar -xzf apache-maven.tar.gz -C $HOME

# -----------------------------
# 4. 環境変数設定
# -----------------------------
export JAVA_HOME=$JDK_INSTALL_DIR/jdk-17.jdk/Contents/Home
export PATH=$JAVA_HOME/bin:$MAVEN_INSTALL_DIR/bin:$PATH
echo "==> JAVA_HOME: $JAVA_HOME"
echo "==> PATH に Maven を追加: $MAVEN_INSTALL_DIR/bin"

# -----------------------------
# 5. pom.xml の Java バージョンを書き換え
# -----------------------------
